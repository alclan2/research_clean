...
# # now let's look at MSLP

# # filter to only origin columns
# og = tc_track[['TID', 'LON_start', 'LAT_start', 'YEAR_start', 'MSLP_start', 'sub_basin_start']]

# #print(og.head())

# # pivot to MSLP mean by sub basin
# mslp = tc_track.pivot_table(
#     index = 'YEAR_start',
...
# plt.plot(mslp.index, mslp[sb], )

# plt.ylabel("MSLP (Pa)")
# plt.xlabel("Year")
# plt.title(f"Average MSLP at TC/TD Origin Node in the {sb}")

# #plt.legend(title="Origin Sub-Basin", bbox_to_anchor=(1.05, 1))
# #plt.xticks(rotation=45)
# plt.tight_layout()
# #plt.savefig(f"images/data_viz/tc_mslp_timeseries_{sb}.png")
# #plt.show()
