...
# load WS yearly csv
ws = pd.read_csv("datasets/SyCLoPS/WS_anom_moving_window_bySubbasin_TC+TD_table.csv")

# load MSLP yearly csv
mslp = pd.read_csv("datasets/MSLP/post-processing/MSLP_anom_moving_window_bySubbasin_table.csv")
...
ws = ws[ws['YEAR'].between(1989,2014)]
sst = sst[sst['year'].between(1989,2014)]

# trim columns and rename so we can join into one table
ws = ws[['YEAR', 'sub_basin_name', 'WS_anom']]
mslp = mslp[['year', 'sub_basin_name', 'mslp_anom']]
sst = sst[['year', 'sub_basin_name', 'mean_anom']]
ws = ws.rename(columns={"YEAR": "year"})
sst = sst.rename(columns={"mean_anom": "sst_anom"})
...
combined = (
    pd.merge(sst, mslp, on=["year", "sub_basin_name"], how="inner")
    .merge(ws, on=["year", "sub_basin_name"], how="inner")
)
...
for sb, group in combined.groupby("sub_basin_name"):
    corr = group[["sst_anom", "mslp_anom", "WS_anom"]].corr()
...
# save to file
#corr_table.to_csv("datasets/data_viz/SST_WS_MSLP_anom_moving window_correl_sb_v2.csv")
...
plt.tight_layout()
#plt.savefig(f"images/data_viz/WSvsMSLPvsSST_anom_moving_window_timeseries_{sb}.png")
plt.show()